...
### Most common choices:
names = ['GJ1214b']
oom       = [7,8,9]  #number of order of magnitudes spanned from surface pressure
...
aerlayers = [50.] # how thick can clouds get
numdays   = [1000] # how many orbits
...
def avgsurface(tau, tirr, tint, gamma):  # EQ 29. CORRECT ONE!
    first = .75 * (tint ** 4) * ((2. / 3.) + tau)
    second = .75 * (tirr ** 4) * f * (
            (2. / 3.) + (1 / (gamma * np.sqrt(3))) + ((gamma / np.sqrt(3)) - (1 / (gamma * np.sqrt(3)))) * np.exp(
        -gamma * tau * np.sqrt(3)))
    final = first + second
    return final ** .25

def makeinitialtprofs(nlay, oom, surfp, tirr, tint, gamma):
    ...
    for i in range(len(taus)):
        initialts[i] = avgsurface(taus[i], tirr, tint, gamma)
    return initialts, p_BAR

def makefort7(oom, nlay, tspd, tdiss, aerlayers, numdays, ntstepin, sponge,
              toaalb, CLOUDS, hazetype, rayscatlam, metallicity, ABSLW, ABSSW, PICKET_FENCE_CLOUDS, lbdrag, lrstrt):

    if CLOUDS:
        cloudstring = 'YESCLOUDS'
    else:
        cloudstring = 'NOCLOUDS'
    gcmname = name + '_' + str(oom) + 'OOM_' + str(nlay) + 'LAYERS_' + str(aerlayers) + 'AERLAYERS_' + str(
        numdays) + 'DAYS_' + str(sponge) + 'SPONGE_' + cloudstring + '_' + hazetype.upper() + 'HAZE_' + str(
        metallicity) + 'METALL'
    ...
    temps, press = makeinitialtprofs(nlay, oom, surfp, tirr, tint, gamma)
