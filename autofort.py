...
### Most common choices:
oom       = 7  #number of order of magnitudes spanned from surface pressure 
...
aerlayers = 10. # how thick can clouds get
numdays   = 200 # how many orbits
ntstepin  = 8 # 8 works for cooler planets, but some will need 4 instead
sponge = 0.005 # sponge layer timescale (in units of planet day). The top layer of the atmosphere gets sponge, then the strength decreases by a factor of ten for each of the next two layers (only the top three layers are affected). 0.005 is a good starting point
toaalb = 0.05
CLOUDS = 'T'
hazetype = 'soot'
...
for i in df['Name']:
    # Looping through planets, making directories to store all fort.7s
    try:
        os.mkdir(destpath+i+'/')
    except:
        pass
...
for pname in df["Name"]:
    ...
    radjup= df["R_p (R_J)"][df['Name']==pname].values[0]
    massjup= df["M_p (M_J)"][df['Name']==pname].values[0]
    pdays= df["P (days)"][df['Name']==pname].values[0]
    rstar= df["R* (R_sun)"][df['Name']==pname].values[0]
    Dau= df["a (au)"][df['Name']==pname].values[0]
    tstar= df["T* (K)"][df['Name']==pname].values[0]
    eccen = df["e"][df['Name']==pname].values[0]
    if type(eccen)==float:
        if eccen < 0.2:
            eccen = 0
#     metal = df["[Fe/H] (dex)"][df['Name']==pname].values[0]
    ...
    try:
        file = open(destpath+pname+"/"+case+"/fort.7", 'x')
        file.write("&COMMENT\n")
        file.write(" THECOMMENT= '{}, {}, {}'\n".format(pname, cloudsyn, magyn))
        file.write(" /\n")
        file.write(" &INPPL      \n")
        ...
        file.write(" /\n")
        file.write(" &INPRN\n")
        file.write(" KRUN     = {},\n".format(tspd*numdays)) # total number of steps
        file.write(" BEGDAY   = 0.000000000000000E+000,\n") # zero for new run, non-zero for restart run
        file.write(" TSPD     = {},\n".format(tspd)) # time steps per day (1 day is one full sideral  2 !pi/ ww)
        file.write(" KITS     = 3,\n") # 3 for initiation, but set this to zero for restart (check)
        file.write(" PNU      = 2.000000000000000E-002,\n")
        file.write(" TDISS    = {},\n".format(tdiss)) # Very important.  Time scale for hyper dissipation on small scale.  Units of planet day.
        file.write(" LFLUX    = T,\n")
        file.write(" BEGDOY   = 0.000000000000000E+000,\n")
        file.write(" NDEL     = 8,\n") # exponent on the hypderdissipation
        file.write(" T0       = {}*1700.,\n".format(nlay)) # Number of layers * initial temperatures
        file.write(" LRSTRT   = {},\n".format(lrstrt))
        ...
        file.write(" KOUNTH  = {},\n".format(tspd*10)) # timesteps between outputting or saving data: History, some multiple of timestep per day, depends on rotation rate, usually 100 days
        file.write(" KOUNTR  = {},\n".format(tspd*10)) # timesteps between outputting or saving data: Restart, some multiple of timestep per day, depends on rotation rate, usually 100 days
        file.write(" KOUNTP  = {},\n".format(tspd)) # set equal to time steps per day
        file.write(" KOUNTE  = 0,\n")
        file.write(" NCOEFF  = 0,\n")
        file.write(" NLAT    = 4,\n")
        file.write(" LGPO    = {}*F,\n".format(nlay)) # integer multiple of number vertical levels
        file.write(" LSPO    = {}*F,\n".format(nlay)) # integer multiple of number vertical levels
        ...
        file.write(" OOM_IN      = {},\n".format(oom)) # Pressure OOM
        file.write(" LPLOTMAP 	= F,\n")
        file.write(" NLPLOTMAP_IN	= 10,\n")
        file.write(" RFCOEFF_IN	= 0.0e-0, \n")
        file.write(" NTSTEP_IN	= {},\n".format(ntstepin)) # how often you run the radiative transfer to redifine the tendencies
        ...
        file.write(" TOAALB		= {}, \n".format(toaalb)) # top of the atmosphere albedo
        ...
        file.write("  NLCR    =           {},\n".format(nlay-1)) # controls which layers are allowed to be convective, set to number of vertical levels -1
        file.write("  NCUTOP  =           5,\n")
        ...
        file.write("  /\n")
        file.write("  &INPRSIJ\n")
        file.write("  TFRC    = {},{},{},{}*0.\n".format(sponge, sponge*10,sponge*100,nlay-3)) # Controls sponge layers


        # Rayleigh friction at each pressure level  time scale (nlayers * 0)
        file.write(" RESTIM  = {}*1e30,\n".format(nlay)) # timescale for newtonian forcing ;;so large, we are not using it
        file.write(" RESTTT =  {} ,\n".format(str(temps)[1:-1])) # important; Temperatures initial conditions from top down to bottom, analytically determined
        file.write(" REDTEP  = {}*0.,\n".format(nlay)) # array nlayers * # (amplitude at each level if newtonian forcing is used)
        ...
        file.write(" ABSSW           = {}, \n".format(ABSSW)) # absorption coefficient for optical cm^2/gram
        file.write(" ABSLW           = {},\n".format(ABSLW)) # IR absorption coefficient at the reference pressure, cm^2/gram
        ...
        file.write(" HAZETYPE     = {},\n".format(hazetype)) #hazetype, so soot, tholin, or clear
        file.write(" MTLX         = .1,\n")
        file.write(" METALLICITY  = {},\n".format(metallicity))
        file.write(" HAZES = F,\n")
        file.write(" PICKET_FENCE_CLOUDS = {},\n".format(PICKET_FENCE_CLOUDS))
        file.write(" MOLEF        = {},\n".format(str(molef)[1:-1])) # Clouds
        file.write(" AERLAYERS    = {},\n".format(aerlayers)) # how many layers a cloud can extend through
