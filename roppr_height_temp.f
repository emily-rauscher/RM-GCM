...
!   Simple cloud test:
      REAL TCON(NL+1),CONDFACT(NL+1),CLOUDLOC(NL+1),AEROPROFX(NL+1)
      INTEGER BASELEV,K

!MnS  Condensation Curve     
...
!  110   CONTINUE
!Jan10      DO 200  J          =   1,NLAYER
        DO 198 J          = NLAYER,1,-1
!       DO 130 L          = 1,NTOTAL
!        WRITE(*,*) min(max(TCON(J)-TT(J),0.0),1.0)
...
198    CONTINUE
        ...
        IF (SUM(CONDFACT).gt.0) THEN 
          ...
          DO 199 J        = BASELEV,1,-1
           AEROPROFX(J)  = AEROPROF(MAXTAULOC-K)
                 K       = K+1
199    CONTINUE
        END IF
          
       DO 200 J           =1,NLAYER
       TAUAER(1,J)       = AEROPROFX(J)*CONDFACT(J)
       WOL(1,J)          = PI0AERSW 
       GOL(1,J)          = ASYMSW
       TAUS(1,J)         = PI0AERSW*AEROPROFX(J)
200   CONTINUE 
...
!    IR
      ...
      DO 202  J          =   1,NDBL,2
              JJ         = J     
       TAUAER(2,JJ)       = AEROPROFX(K)*EXTFACTLW*CONDFACT(K)
       WOL(2,JJ)          = PI0AERLW
       GOL(2,JJ)          = ASYMLW
