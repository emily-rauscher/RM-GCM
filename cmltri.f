...
!      write(*,*) 'LPLOT', LPLOTMAP, OOM_IN

!      IF(LPLOTMAP) THEN
C Open PGPLOT graphics device.
      IF (PGOPEN('/xwin') .LT. 0) STOP 
      CALL PGASK(.FALSE.)   ! Remove each page query
C
!      ENDIF
...
!                  TMPLAT=-PI/180.0*ALAT(IH)
               ...
               ENDIF
                 ...
                 TNLG(IP+I)=TNLG(IP+I) + DTEP *FAC(L)
     & *COS(1*TMPLAT) 
     & *COS(1.0*PI2*(I-1)/MG)/1.0 !*RANF(IDUM)


C&&&& Part for NS burst localized forcing &&&&&&&
c                 TNLG(IP+I)=(TRS(L)-T0(L)) !*RANF(IDUM)
c                 TNLG(IP+I)=TNLG(IP+I) + DTEP *FAC(L)
c     & *exp(  (-(1*TMPLAT - 0.5*PI2/4)**2 
c     &- (1.0*PI2*(I-1)/MG -0.25*PI2)**2) 
c     &  / (PI2/40)**2)
C&&&&&&&&&&&&
