...
!            CLEAR CASE
               B3CLR(L,J)     =  0.5*(1.-SQ3*G0CLR(L,J)*U0)
               B4CLR          =  1. - B3CLR(L,J)
               X2CLR          =  TAULCLR(L,J)*DU0
               EE3CLR(L,J)    =  EXP(-X2CLR)
               X3CLR          =  OPDCLR(L,J)*DU0
               EL3CLR(L,J)    =  EXP(-X3CLR)*SOL(L)
...
!           CLEAR CASE
               ...
               DIRECTCLR(L,J) = U0*EL3CLR(L,J)
               C1CLR          =  B1CLR(L,J) - DU0
               if( ABS(C1CLR).lt.EPSILON ) THEN
               c1CLR = SIGN(EPSILON,C1CLR)
               endif
               C2CLR          =  AKCLR(L,J)*AKCLR(L,J) - DU0*DU0
               if( ABS(C2CLR) .le. EPSILON ) then
               c2CLR = EPSILON
               endif
               CP1CLR         =  W0(L,J)*(B3(L,J)*C1+B4*B2(L,J))/C2
               CPBCLR(L,J)    =  CP1CLR * EL3CLR(L,J)
               if( j .ne. 1 ) then
                 x4 = eL3(L,j1)
               else
                 x4 = soL(L)
